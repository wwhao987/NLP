...
from intention_identification.data_helper import load_train_eval_dataloader, fetch_tokens_from_text
from intention_identification.metrics import accuracy
from intention_identification.models import TextClassifyModel
from intention_identification.utils import create_dir
...
class Trainer(object):
    def __init__(self, token_vocab_file, label_vocab_file, output_dir):
        ...
        create_dir(self.output_model_dir)
        create_dir(self.output_summary_dir)

    def train_loop(self, epoch, batch_step, dataloader, model, loss_fn, opt, log_interval_batch, writer):
        model.train()  # 进入训练阶段
        losses = []
        accuracies = []
        for _, _, batch_seq_idxes, batch_seq_length, _, batch_label_id in dataloader:
            ...
            loss = loss_fn(score, batch_label_id)
    ...
    def eval_loop(self, epoch, batch_step, dataloader, model, loss_fn, log_interval_batch, writer):
        model.eval()  # 进行评估推理阶段
        losses = []
        accuracies = []
        for _, _, batch_seq_idxes, batch_seq_length, _, batch_label_id in dataloader:
    ...
    def training(self, cfg, data_files, n_epochs, batch_size, eval_ratio, save_interval_epoch, log_interval_batch,
        ...
        # 2. 构建相关模型
        model = TextClassifyModel.build_model(cfg, weights).to(self.device)
        LOGGER.info(f"{model}")
        ...
        with SummaryWriter(log_dir=self.output_summary_dir) as writer:
            ...
            for epoch in range(start_epoch, n_epochs):
                # 训练
                train_batch_step = self.train_loop(
                    epoch, train_batch_step, train_dataloader, model, loss_fn, opt, log_interval_batch, writer
                )
                # 评估
                eval_batch_step = self.eval_loop(
                    epoch, eval_batch_step, eval_dataloader, model, loss_fn, log_interval_batch, writer
                )
                # 模型保存
...
class Evaluator(object):
    ...
    def eval(self, data_files, batch_size):
        ...
        with open(os.path.join(self.eval_output_dir, "eval.txt"), "w", encoding='utf-8') as writer:
            for batch_text, batch_tokens, batch_seq_idxes, batch_seq_length, batch_label, batch_label_id in tqdm(
                    eval_dataloader):
                batch_seq_idxes = batch_seq_idxes.to(self.device)
                batch_seq_length = batch_seq_length.to(self.device)
                batch_label_id = batch_label_id.to(self.device)
                ...
                bs = len(batch_text)
                predict_label_id = predict_label_id.detach().numpy()
                predict_label = self.label_vocab.lookup_tokens(list(predict_label_id))
                batch_seq_idxes = batch_seq_idxes.detach().numpy()
                for i in range(bs):
...
class Predictor(object):
    ...
    def predict(self, text: str, k: int = 1, probability_threshold: float = 0.1, is_debug: bool = False):
        ...
        # tensor结果转换numpy
        topk_probability = topk_probability.detach().numpy()
        topk_label_idxes = topk_label_idxes.detach().numpy()
